...
[[nodiscard]]
char* memory::TrampHook(void* _src, void* _dst, size_t size) {

    if (size < 5) {
        return 0;
    }

    auto src = reinterpret_cast<char *>(_src);
    auto dst = reinterpret_cast<char *>(_dst);
...
[[nodiscard]]
unsigned char* memory::FindPattern(void* _base_addr, size_t img_size, void* _pattern, size_t pattern_size) {

    auto base_addr = reinterpret_cast<unsigned char*>(_base_addr);
    auto pattern = reinterpret_cast<unsigned char*>(_pattern);
